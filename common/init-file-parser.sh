...
# keys_without_section_warning - should we warn, when keys have no section         #
...
declare keys_without_section_warning
...
# local_keys_without_section_warning - should we warn, when keys have no section   #
...
local_keys_without_section_warning=true
...
function setup_global_variables
{
    ...
    if [[ -n "${keys_without_section_warning}" ]] && [[ "${keys_without_section_warning}" = false || "${keys_without_section_warning}" = true ]]; then
         local_keys_without_section_warning=$keys_without_section_warning
    fi
    ...
    while read -r line; do
        ...
        if [[ $line =~ ^"["(.+)"]"$ ]]; then                                   # Match pattern for a 'section'
            ...
            if [[ -z ${key} ]]; then
                ...
                if [[ "${section}" == "${DEFAULT_SECTION}" ]] && [[ "${local_keys_without_section_warning}" = true ]]; then
